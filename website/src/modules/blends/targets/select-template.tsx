...
const SelectTargetTemplate = () => {
  ...
  const addTarget = () => {
    ...
    const templateId = Number(selectedTemplate.template_id);
    const f = targets.filter((t) => t.template === templateId);

    if (f.length > 0) {
      // TODO: show error message that target already exists
      return;
    }
    ...
    const target: TargetProps = {
      ...
      template: templateId,
      chance: targets.length > 0 ? 0 : 100 // 0 if there are one or more, otherwise 100
    };

    dispatchTargets({ type: 'add', data: target });
  };
...
export default SelectTargetTemplate;
